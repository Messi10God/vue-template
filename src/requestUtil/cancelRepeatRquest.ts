...
import type { AxiosRequestConfig, AxiosResponse, Canceler, AxiosError } from 'axios';
import { generateReqKey } from './commonFuns';

// addPendingRequest ：用于把当前请求信息添加到pendingRequest对象 中；
const pendingRequest = new Map<string, Canceler>(); // Map对象保存键值对。任何值(对象或者原始值) 都可以作为一个键或一个值。
...
export function removePendingRequest(response: AxiosResponse | AxiosError) {
